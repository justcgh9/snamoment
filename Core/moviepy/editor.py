...
# FX
import Core.moviepy.video.fx.all as vfx
...
for method in [
    ...
    exec(f"VideoClip.{method.split('.')[1]} = {method}")
...
for method in ["afx.audio_fadein",
               ...
               "afx.audio_normalize",
               "afx.volumex"]:
    exec(f"AudioClip.{method.split('.')[1]} = {method}")
...
try:
    from Core.moviepy.video.io.preview import show, preview
except ImportError:
...
try:
    from Core.moviepy.audio.io.preview import preview
except ImportError:
    def preview(self, *args, **kwargs):
        """ NOT AVAILABLE : clip.preview requires Pygame installed."""
        raise ImportError("clip.preview requires Pygame installed")
