...
from Core.moviepy.decorators import (apply_to_audio, apply_to_mask,
...
class Clip:
    ...
    def subclip(self, t_start=0, t_end=None):
        ...
        if (self.duration is not None) and (t_start > self.duration):
            raise ValueError(f"t_start ({t_start}) should be smaller "
                             f"than the clip's duration ({self.duration})")
        ...
        if (t_end is None) and (self.duration is not None):
            ...
            if self.duration is None:
                print(f"Error: subclip with negative times (here {(str((t_start, t_end)))})"
    ...
    def iter_frames(self, fps=None, with_times=False, logger=None, dtype=None):
        ...
        for t in logger.iter_bar(t=np.arange(0, self.duration, 1.0 / fps)):
            if t * 10 % 5 == 0:
                if(self.progressChangeListener!=None):
                    self.progressChangeListener(t*10)
                print(str(t * 10) + "%")
