...
class VideoClip(Clip):
    ...
    def write_videofile(self, filename, fps=None, codec=None,
                        ...
                        preset="medium", audio_nbytes=4, audio_codec=None,
        ...
        fps
          ...
          provided, and the clip has a fps attribute, this fps will be used.
        ...
        temp_audiofile
          ...
          incorporated in the movie, if any.
        ...
        audio_bitrate
          ...
          necessarily be the same in the final file.
        ...
        if codec is None:
            ...
            try:
                ...
                raise ValueError("MoviePy couldn't find the codec associated "
                                 ...
                                 "parameter in write_videofile.") from error
...
#            audio_codec = 'pcm_s32le'
        ...
        make_audio = (audiofile is None and audio and self.audio is not None)
        ...
        if make_audio and temp_audiofile:
            ...
            audiofile = name + Clip._TEMP_FILES_PREFIX + f"wvf_snd.{audio_ext}"
    ...
    def write_images_sequence(self, nameformat, fps=None, verbose=True,
        ...
        """
        logger = proglog.default_bar_logger(logger)
        logger(message=f'Moviepy - Writing frames {nameformat}.')
        ...
        logger(message=f'Moviepy - Done writing frames {nameformat}.')
    ...
    def subfx(self, fx, ta=0, tb=None, **kwargs):
        ...
        >>> # played twice slower in ``newclip``
        ...
        from Core.moviepy.video.compositing.concatenate import concatenate_videoclips
...
class TextClip(ImageClip):
    ...
    method
      Either 'label' (default, the picture will be autosized to fit
    ...
    def __init__(self, txt=None, filename=None, size=None, color='black',
        ...
        if interline is not None:
            cmd += ["-interline-spacing", str(interline)]

        if tempfilename is None:
            tempfile_fd, tempfilename = tempfile.mkstemp(suffix='.png')
            os.close(tempfile_fd)

        cmd += [f"{method}:{txt}", "-type", "truecolormatte", "PNG32:%s" % tempfilename]
        ...
        try:
            ...
            error = (f"MoviePy Error: creation of {filename} failed because of the "
                     f"following error:\n\n{str(err)}.\n\n."
    ...
    def list(arg):
        ...
        if arg == 'font':
            return [line.decode('UTF-8')[8:] for line in lines if line.startswith(b"  Font:")]
        elif arg == 'color':
            return [line.split(b" ")[0] for line in lines[2:]]
