...
from Core.moviepy.tools import extensions_dict
    ...
    **html_kwargs
    ...
    if filetype in ['audio', 'video']:
        ...
        if duration > maxduration:
            raise ValueError(f"The duration of video {filename} ({duration}) exceeds "
                             f"the 'maxduration' attribute. You can increase 'maxduration', "
                             f"by passing 'maxduration' parameter to ipython_display function. "
                             f"But note that embedding large videos may take all the memory away!")
