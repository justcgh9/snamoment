...
import pygame as pg
from Core.moviepy.decorators import convert_masks_to_RGB, requires_duration
...
def show(clip, t=0, with_mask=True, interactive=False):
    ...
    if with_mask and (clip.mask is not None):
        import Core.moviepy.video.compositing.CompositeVideoClip as cvc
        ...
        while True:
            for event in pg.event.get():
                if event.type == pg.KEYDOWN:
                    ...
                    print(f"position, color : {str((x, y))}, {str(rgb)}")
            time.sleep(.03)
    ...
    for t in np.arange(1.0 / fps, clip.duration - .001, 1.0 / fps):
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                print(f"time, position, color : {t}, {str((x, y))}, {str(rgb)}")
