...
from Core.moviepy.compat import DEVNULL, PY3
from Core.moviepy.config import get_setting
    ...
    def write_frame(self, img_array):
        ...
        try:
            ...
            error = (str(err) + (f"\n\nMoviePy error: FFMPEG encountered "
                                 f"the following error while writing file {self.filename}:"
                                 f"\n\n {str(ffmpeg_error)}"))

            if b"Unknown encoder" in ffmpeg_error:
                error = error + (f"\n\nThe video export "
                                 f"failed because FFMPEG didn't find the specified "
                                 f"codec for video encoding ({self.codec}). Please install "
                                 f"this codec or change the codec when calling "
                                 f"write_videofile. For instance:\n"
                                 f"  >>> clip.write_videofile('myvid.webm', codec='libvpx')")

            elif b"incorrect codec parameters ?" in ffmpeg_error:
                error = error + (f"\n\nThe video export "
                                 f"failed, possibly because the codec specified for "
                                 f"the video ({self.codec}) is not compatible with the given "
                                 f"extension ({self.ext}). Please specify a valid 'codec' "
                                 f"argument in write_videofile. This would be 'libx264' "
                                 f"or 'mpeg4' for mp4, 'libtheora' for ogv, 'libvpx for webm. "
                                 f"Another possible reason is that the audio codec was not "
                                 f"compatible with the video codec. For instance the video "
                                 f"extensions 'ogv' and 'webm' only allow 'libvorbis' (default) as a"
                                 f"video codec.")
