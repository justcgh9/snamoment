...
from Core.moviepy.decorators import use_clip_fps_by_default
...
class FramesMatch:
    ...
    def __str__(self):
        return f'({self.t1}, {self.t2}, {self.d_min}, {self.d_max})'
...
class FramesMatches(list):
    ...
    def write_gifs(self, clip, gif_dir):
        for (start, end, _, _) in self:
            name = f"{gif_dir}/{100 * start}_{100 * end}.gif"
