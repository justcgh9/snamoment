...
from Core.moviepy.tools import cvsecs
...
def use_clip_fps_by_default(f, clip, *a, **k):
    ...
    def fun(fps):
        if fps is not None:
            ...
            return clip.fps
        raise AttributeError(f"No 'fps' (frames per second) attribute specified"
                             f" for function {f.__name__} and the clip has no 'fps' attribute. Either"
                             f" provide e.g. fps=24 in the arguments of the function, or define"
                             f" the clip's fps with `clip.fps=24`")
