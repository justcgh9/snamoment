...
from Core.moviepy.audio.io.ffmpegaudiowriter import ffmpeg_audiowrite
from Core.moviepy.Clip import Clip
from Core.moviepy.decorators import requires_duration
from Core.moviepy.tools import deprecated_version_of, extensions_dict
...
class AudioClip(Clip):
    ...
    def max_volume(self, stereo=False, chunksize=50000, logger=None):
        ...
        for chunk in self.iter_chunks(chunksize=chunksize, logger=logger):
            maxi = np.maximum(maxi, abs(chunk).max(axis=0)) if \
                stereo else max(maxi, abs(chunk).max())
        return maxi
        ...
        if codec is None:
            ...
            try:
                ...
                raise ValueError("MoviePy couldn't find the codec associated "
                                 ...
                                 "parameter in write_audiofile.") from err
