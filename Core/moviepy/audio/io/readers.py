...
from Core.moviepy.compat import DEVNULL
from Core.moviepy.config import get_setting
from Core.moviepy.video.io.ffmpeg_reader import ffmpeg_parse_infos
...
class FFMPEGAudioReader:
    ...
    def __init__(self, filename, buffersize, print_infos=False,
        ...
        self.nbytes = nbytes
        self.fps = fps
        self.f = f's{8 * nbytes}le'
        self.acodec = f'pcm_s{8 * nbytes}le'
    ...
    def get_frame(self, tt):
        ...
        if isinstance(tt, np.ndarray):
            ...
            if not in_time.any():
                raise IOError(f"Error in file {self.filename}, Accessing time "
                              f"t={tt[0]}-{tt[-1]} seconds, with clip "
            ...
            try:
                ...
                warnings.warn(f"Error in file {self.filename}, At time t={tt[0]}-{tt[-1]} seconds,"
                              f" indices wanted: {indices.min()}-{indices.max()}, "
