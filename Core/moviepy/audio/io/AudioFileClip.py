...
from Core.moviepy.audio.AudioClip import AudioClip
from Core.moviepy.audio.io.readers import FFMPEGAudioReader
...
class AudioFileClip(AudioClip):
    ...
    Note that this creates subprocesses and locks files.
    If you construct one of these instances, you must call
    close() afterwards, or the subresources will not be cleaned up until the process ends.

    If copies are made, and close() is called on one, it may cause
    methods on the other copies to fail.
    ...
    def __init__(self, filename, buffersize=200000, nbytes=2, fps=44100):
        ...
        self.reader = FFMPEGAudioReader(filename, fps=fps, nbytes=nbytes,
