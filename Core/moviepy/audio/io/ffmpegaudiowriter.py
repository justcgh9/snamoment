...
from Core.moviepy.decorators import requires_duration
    ...
    def write_frames(self, frames_array):
        try:
            ...
            error = (str(err) + (f"\n\nMoviePy error: FFMPEG encountered the following error "
                                 f"while writing file {self.filename}:\n\n{str(ffmpeg_error)}"))
...
def ffmpeg_audiowrite(clip, filename, fps, nbytes, buffersize,
    ...
    """
    A function that wraps the FFMPEGAudioWriter to write an AudioClip
    to a file.
    ...
    writer = FFMPEGAudioWriter(filename, fps, nbytes, clip.nchannels,
